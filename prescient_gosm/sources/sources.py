...
import statdist.copula as copula
from statdist import UnivariateEmpiricalDistribution
from statdist import UnivariateEpiSplineDistribution
import prescient_gosm.derivative_patterns.derivative_patterns as dp
from prescient_gosm.markov_chains.states import State
from prescient_gosm.sources.segmenter import Criterion
...
class Source:
    ...
    def __init__(self, name, dataframe, source_type):
        ...
        # We sort the data by datetime if unsorted data is passed in
    ...
    def rolling_window(self, day, historic_data_start=None,
                       historic_data_end=None, time_step=None,
        ...
        and the dayahead data is the data from the day up to the day plus
        the planning_period_length.
        ...
        Args:
            ...
            time_step (str): The time step size of the underlying data.
            planning_period_length (str): The length of the planning period for
                the underlying data. This specifies how long the dayahead dataframe will be.
        Returns:
            RollingWindow: The rolling window of data
        """

        if (time_step is None) or (planning_period_length is None):
            ...
            # If the time step and the planning period length are set, use these
            # information for creating the dataframes. Planning period length defines,
            # how much data is included in the dayahead dataframe. Time step
            # specifies the time step in the underlying data.

            # If start not specified, we take the first date in dataframe
            if historic_data_start is None:

                historic_data_start = self.data.index[0]
            # If end not specified, we take last time step before the passed
            # in day
            if historic_data_end is None:
                historic_data_end = day - pd.Timedelta(time_step)
            historic_frame = self.data[historic_data_start:historic_data_end]
            dayahead_frame = self.data[day:day + pd.Timedelta(planning_period_length)]
            #if (self.source_type == "load"):
            #    print("Data", self.data)
            #    print("Day: ", day)
            #    print("Dayahead frame: ", dayahead_frame)
...
class ExtendedSource(Source):
    ...
    Attributes:
        ...
        time_step (str): A string in the pandas offset aliases format for the
            desired frequency of the data. Note, that this does not have to
            be the actual frequency of the data. (If both are different, an
    ...
    def __init__(self, source, criteria=None, bounds=None,
                 diurnal_pattern=None, source_params=None, time_step=None):
        """
        Args:
            ...
            diurnal_pattern (pd.Series): A timeseries specifying the diurnal
                pattern for solar sources. This should be specified only if the
                source type is 'solar'
            time_step (str): A string in the pandas offset aliases format for
                the desired frequency of the data. Note, that this does not
                have to be the actual frequency of the data. (If both are
    ...
    def check_time_step(self):
        """
        This method checks, if the time step attribute and the frequency of
        datetime index of the source's data are consistent.
        """
        time_step = self.time_step

        if time_step is None:
            raise ValueError('You have to include a time step for each source '
                             'in the sourcelist  file!')

        if pd.infer_freq(self.data.index) is None:
            if pd.infer_freq(self.data.index[:5]) is None:
                raise ValueError('Infering the frequency of the datetime index'
                                 ' is not possible.')
            else:
                freq = pd.infer_freq(self.data.index[:5])
        else:
        ...
        if freq != time_step:
            raise ValueError('The frequency of your data is not consistent '
                             'with the provided time step.')
    ...
    def rolling_window(self, day, historic_data_start=None,
        ...
        Args:
            ...
            time_step (str): The time step size of the underlying data.
            planning_period_length (str): The length of the planning period
                for the underlying data. This specifies how long the dayahead dataframe will be.
        Returns:
        ...
        window = Source.rolling_window(self, day, historic_data_start,
                                       historic_data_end, time_step, planning_period_length)

        cap = self.capacity(day)

        #if the source type is solar we pass in the average sunrise and sunset
        #   to the window corresponding to the day passed in
        if self.source_type == 'solar':
            avg_sunrise_sunset = self.estimate_sunrise_sunset(day)
        else:
            avg_sunrise_sunset = None

        return ExtendedWindow(window, self.criteria, cap,
            self.diurnal_pattern, self.source_params, time_step=self.time_step,
            avg_sunrise_sunset=avg_sunrise_sunset)
    ...
    def daylight_data(self, day, historic_data_start=None,
    ...
    def __str__(self):
        ...
        if self.source_type == 'solar':
            string += 'Average Sunrise and Sunset({})\n'.format(self.avg_sunrise_sunset)
        return string
...
class RollingWindow(Source):
    ...
    def interpolate(self, column_name):
        ...
        Args:
            column_name (str): name of the column to interpolate
        """
        Source.interpolate(self, column_name)
    ...
    def mask_data(self, time):
        """
        Create a new rolling window based on the data of the original
            rolling window instance. The purpose of this function is
            to have a rolling window with just historical data for a
            certain point in time.

        Args:
            time (datetime-like): The historical data of the new
                rolling window will just have data about this time

        """

        historic_data = self.historic_data
        hour_mask = time.hour == historic_data.index.hour
        historic_data = historic_data[hour_mask]
        minute_mask = time.minute == historic_data.index.minute
        historic_data = historic_data[minute_mask]
        second_mask = time.second == historic_data.index.second
        historic_data = historic_data[second_mask]

        rolling_window = RollingWindow(self.name, historic_data, self.source_type, self.dayahead_data)
...
class ExtendedWindow(RollingWindow):
    ...
    Attributes:
        ...
        diurnal_pattern (pd.Series): A timeseries specifying the diurnal
            ...
            source type is 'solar'
        time_step (str): The time step size of the underlying data
        source_params (dict): This dictionary will
            specify additional data relevant to a source.
        avg_sunrise_senset (Tuple(float)): The two values for the average
            sunrise and sunset for solar source at the current scenario day

    """
    def __init__(self, window, criteria, capacity,
                 diurnal_pattern=None, source_params=None, time_step=None, avg_sunrise_sunset=None):
        ...
        self.diurnal_pattern = diurnal_pattern
        self.time_step = time_step
        self.avg_sunrise_sunset = avg_sunrise_sunset
    ...
    def get_copy(self):
        """
        This function returns a copy of the rolling window instance

        """
        window = RollingWindow(self.name, self.historic_data.copy(), self.source_type, self.dayahead_data.copy())
        return ExtendedWindow(window, self.criteria, self.capacity,
            self.diurnal_pattern, self.source_params, time_step=self.time_step,
            avg_sunrise_sunset=self.avg_sunrise_sunset)

    def daylight_data(self, verbose=True):
        """
        This function takes an instance of a rolling window and copies the
        dayahead_data and historic_data and purges the nightly hours of
        the data for solar sources

        """

        # Check to see if we can appropriately partition daylight hours

        if self.avg_sunrise_sunset is None and self.scenario_day is None:
            raise ValueError("There is no scenario day set to aquire average sunrise and sunset")
        elif self.avg_sunrise_sunset is None:
            if verbose:
                print("Warning: Estimating "
                      "sunrise and sunset using average of past data."
                      .format(self.name), file=sys.stderr)
                self.avg_sunrise_sunset = Source.estimate_sunrise_sunset(self.scenario_day)

        self.daylight_dayahead = self.dayahead_data.copy()
        self.daylight_historic = self.historic_data.copy()

        for index1, row1 in self.daylight_historic.iterrows():
            if (index1.hour < self.avg_sunrise_sunset[0]) or (index1.hour >= self.avg_sunrise_sunset[1]):
                self.daylight_historic.drop(index1, inplace=True)

        for index2, row2 in self.daylight_dayahead.iterrows():
            if (index2.hour < self.avg_sunrise_sunset[0]) or (index2.hour >= self.avg_sunrise_sunset[1]):
                self.daylight_dayahead.drop(index2, inplace=True)
