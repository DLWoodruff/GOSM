...
import prescient_gosm.parsers.tokenizer as tokenizer
from prescient_gosm.sources import Source, ExtendedSource
from prescient_gosm.sources.segmenter import parse_segmentation_file
from prescient_gosm.sources.upper_bounds import parse_upper_bounds_file
from prescient_gosm.sources.upper_bounds import parse_diurnal_pattern_file
from prescient_gosm.sources.source_args import source_args
from prescient_gosm.parsers.tokenizer import L_PAREN, R_PAREN, COMMA, SEMICOLON
from prescient_gosm.parsers.tokenizer import EQUALS, ALPHANUM, QUOTED_STRING
...
class ParsedSource:
    ...
    def _validate(self):
        ...
        for key in source_args.required_args():
            if key not in self.params:
                raise ValueError("Source {} has no {} key".format(
                    self.name, key))
        # We check if all keys are recognized
        for key in self.params:
            if key not in source_args.args:
                raise ValueError("Source {} has unrecognized key {}".format(
                    self.name, key))

    def initialize_args(self):
        """
        This will initialize all of the arguments either setting them to the
        default or calling the initialization function on them.
        """
        for name, arg in source_args.args.items():
            if name in self.params:
                try:
                    self.params[name] = arg.initialize(self.params[name])
                except ValueError as e:
                    print("The key {} for source {} was not properly set"
                            .format(name, self.name), file=sys.stderr)
                    print("Was given {}={}".format(name, self.params[name]))
                    print("Error: {}".format(e.args[0]), file=sys.stderr)
                    sys.exit(1)
            else:
                self.params[name] = arg.default
    ...
    def to_source(self):
        ...
        if self.params.get('forecasts_as_actuals'):
            if self.params.get('is_deterministic', default=['is_deterministic']):
        ...
        if self.params['segmentation_file'] is not None:
        ...
        if self.params['capacity_file'] is not None:
        ...
        if self.params['diurnal_pattern_file'] is not None:
        ...
        if self.params['aggregate']:
            if self.params['disaggregation_file'] is None:
                raise ValueError("The aggregate option is set to True for "
                                 "source {} but 'disaggregation_file' is not "
                                 "set".format(self.name))
            df = pd.read_csv(self.params['disaggregation_file'],
                             index_col='source', comment='#')
            disaggregation = dict(df['proportion'])
            self.params['disaggregation'] = disaggregation

        # Parse out the time step of the data.
        if self.params['time_step']:
...
def sources_from_new_sources_file(filename):
    ...
    sources files designed as of Nov 2017. It will return ExtendedSource
