...
import compute_multi_interval_3
import argparse
import pandas as pd
...
def count(number, daylist_in, series, num_processes=15):
    """
    ...
    with mp.Pool(num_processes) as pool:
        ...
        dayruns = [pool.apply_async(compute_multi_interval_3.main, (future_time,))
...
def main():
    ...
    fd, daylist, series = set_list(args.data_file,
                                   compute_multi_interval_3.f_time(),
    ...
    with open('../../research_results/evaluation_multi' + '_'
                      + args.date_time_no + '_' + str(args.hour_range_size)
                      + '_' + str(win) + sepa + '.txt', 'w') as output:
        ...
        print('Window size:', args.ws, file = output)
