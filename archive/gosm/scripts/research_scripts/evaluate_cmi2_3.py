...
import compute_multi_interval2_3
import argparse
import pandas as pd
...
def count(number, daylist_in, series, num_processes = 2):
    """
    ...
    with mp.Pool(num_processes) as pool:
        ...
        dayruns = [pool.apply_async(compute_multi_interval2_3.main,
...
def main():
    ...
    fd, daylist, series = set_list(args.data_file,
                                   compute_multi_interval2_3.f_time(),
    ...
    with open('../../research_results/evaluation_multi2_' + args.date_time_now
                      + '_' + str(args.hour_range_size) + '_' + str(win) + sepa + '.txt', 'w') as output:
