...
import gosm.sources.segmenter as segmenter
...
def compute_interval(source, dt, alpha, hour_range_size, capacity):
    ...
    #print(len(errors))
    number_historic_hours = len(errors)
    distr = distributions.UnivariateEmpiricalDistribution.fit(errors)
    #distr.plot(plot_pdf=True, plot_cdf=True)
...
if __name__ == '__main__':
    args = parser.parse_args()
    lower, upper, future_time, number = main(f_time(), args.ws)
